...
simu_vpcl_ar_diff_lmer_se <- function(df = simu_df){
  ...
  vpcl<- data.frame(Reduce(rbind, vipcal_vp))
  #vpcl<- data.frame(t(sapply(vipcal_vp, c)))
  colnames(vpcl)<- c('Expt_No', 'Sample_Type', 'VP', 'VP_SE')
...
simu_calc_funct_list<- list(simu_lm_ap, simu_lm_sr_avg, simu_lm_ar,
 ...
 simulation_function<- function(input_df = simu_df, output_df = output_df, output_csv = T, output_dir = './simulations',
   ...
   #sink(paste0('./simulations/logs/simu_output_log_method.txt'))
   ...
   tryCatch(
     ...
     for (mtd in 1:12){
       ...
       try(output_df<- output_df %>%
             full_join(simu_calc_funct_list[[mtd]](input_df)))
       .GlobalEnv$output_df_temp<- output_df
